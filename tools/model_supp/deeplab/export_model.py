...
flags.DEFINE_bool('output_prob', False,
                  'Output probability map or not')
...
def main(unused_argv):
  ...
  WARNING
  -------------
  changed to output probability map for each class
  FIXED ONLY for single-scale inference, WILL NOT work for multi-scale inference
  """
  ...
  with tf.Graph().as_default():
    ...
    if tuple(FLAGS.inference_scales) == (1.0,):
      ...
      predictions = model.predict_labels(
          ...
          ret_prob_output=True)
      predictions[common.OUTPUT_TYPE] = tf.nn.softmax(predictions[common.OUTPUT_TYPE])
    else:
