...
class DeepCrackDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if w > 0 or h > 0:
            ...
            modh = int(math.ceil(ori_h / 64.0) * 64)
            modw = int(math.ceil(ori_w / 64.0) * 64)
            img = cv2.resize(img, (modw, modh), interpolation=cv2.INTER_CUBIC)
            lab = cv2.resize(lab, (modw, modh), interpolation=cv2.INTER_CUBIC)
        ...
        return {'image': img, 'label': lab,
                'A_paths': img_path, 'B_paths': lab_path,
